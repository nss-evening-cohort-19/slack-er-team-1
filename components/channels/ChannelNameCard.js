      ...
      <ul>
        {allChannels?.map((channel) => (
          <li key={channel.firebaseKey}>
            #{channel.channelName}{' '}
            <button id={channel.firebaseKey} type="button" className={user.uid === channel.uid ? 'btn btn-outline-light' : 'btn btn-outline-success'} onClick={user.uid === channel.uid ? deleteTheJoinObj : handleSubmit}>
              {user.uid === channel.uid ? 'Leave' : 'Join'}
            </button>
          </li>
        ))}
      </ul>
